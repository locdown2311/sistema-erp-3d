...
const form = useForm({
    ...
    base_price: props.product.base_price || '0.00',
    base_cost: props.product.base_cost || '0.00',
    print_time_hours: props.product.print_time_hours || '',
...
const toggleExtraImageDelete = (id) => {
...
const submitVariation = () => {
    variationForm.post(`/produtos/${props.product.id}/variacoes`, {
        preserveScroll: true,
...
const deleteVariation = (variationId) => {
    if (confirm('Tem certeza que deseja remover esta variação?')) {
        router.delete(`/produtos/${props.product.id}/variacoes/${variationId}`, {
            preserveScroll: true,
        });
    }
};
    ...
    <AppLayout>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-6xl">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden h-fit mb-6 lg:mb-0">
                ...
                <form @submit.prevent="submit" class="p-6">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" v-model="form.category" list="categories" 
                               class="w-full px-4 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg text-sm text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow outline-none"
                    ...
                    <div class="mb-5">
                        <ImageUpload :current-image="product.image_path" :image-url="product.thumbnail_url" @update:croppedData="handleImageUpdate" />
                    ...
                    <div class="mb-5">
                        ...
                        <div v-if="product.images && product.images.length > 0" class="grid grid-cols-3 sm:grid-cols-4 gap-3 mb-3">
                            <div v-for="img in product.images" :key="img.id" class="relative group">
                                ...
                                <label class="absolute top-1.5 right-1.5 cursor-pointer" title="Marcar para remover">
                                    <input type="checkbox" :value="img.id" @change="toggleExtraImageDelete(img.id)" class="sr-only peer">
                        ...
                        <p v-if="product.images && product.images.length > 0" class="text-xs text-zinc-500 mb-3">Marque o <strong>X</strong> nas imagens que deseja remover.</p>
                    ...
                    <div class="mb-5">
                        ...
                        <textarea v-model="form.description" rows="3" 
                                  class="w-full px-4 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg text-sm text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow outline-none resize-none"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" v-model="form.weight_grams" step="0.01" min="0" 
                    ...
                    <div class="flex items-center gap-3 pt-6 border-t border-zinc-200 dark:border-zinc-800">
                        <button type="submit" :disabled="form.processing" class="inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 dark:focus:ring-offset-zinc-900 disabled:opacity-50 disabled:cursor-not-allowed">
                            <i class="fas fa-check" :class="{ 'fa-spinner fa-spin': form.processing }"></i> Salvar Alterações
                        </button>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden h-fit">
                ...
                <div class="p-6">
                    <div v-if="product.variations && product.variations.length > 0" class="overflow-x-auto border border-zinc-200 dark:border-zinc-800 rounded-lg mb-8">
                        <table class="w-full text-left text-sm whitespace-nowrap">
                            ...
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800 text-zinc-700 dark:text-zinc-300">
                                <tr v-for="variation in product.variations" :key="variation.id" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <button @click="deleteVariation(variation.id)" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-500/10 transition-colors" title="Excluir">
                    ...
                    <div v-else class="text-center py-6 mb-8 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-lg bg-zinc-50 dark:bg-zinc-900/50">
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">Nenhuma variação cadastrada</p>
                    </div>

                    <form @submit.prevent="submitVariation" class="bg-zinc-50 dark:bg-zinc-900/50 p-5 rounded-lg border border-zinc-200 dark:border-zinc-800">
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Nome da Variação</label>
                            <input type="text" v-model="variationForm.name" required placeholder="Ex: Azul, Grande, PLA+" 
                                   ...
                                   :class="{ 'border-red-500': variationForm.errors.name }">
                            <div v-if="variationForm.errors.name" class="mt-1 text-xs text-red-500">{{ variationForm.errors.name }}</div>
                        </div>
                        
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Tipo</label>
                                <select v-model="variationForm.type" 
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" v-model="variationForm.price_modifier" step="0.01" 
                                           ...
                                           :class="{ 'border-red-500': variationForm.errors.price_modifier }">
                                </div>
                                <div v-if="variationForm.errors.price_modifier" class="mt-1 text-xs text-red-500">{{ variationForm.errors.price_modifier }}</div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-5">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" v-model="variationForm.cost_modifier" step="0.01" 
                                           class="w-full pl-8 pr-3 py-2 bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 rounded-lg text-sm text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow outline-none"
                                           :class="{ 'border-red-500': variationForm.errors.cost_modifier }">
                                </div>
                                <div v-if="variationForm.errors.cost_modifier" class="mt-1 text-xs text-red-500">{{ variationForm.errors.cost_modifier }}</div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">SKU</label>
                                <input type="text" v-model="variationForm.sku" placeholder="Opcional" 
                                       ...
                                       :class="{ 'border-red-500': variationForm.errors.sku }">
                            </div>
                        </div>
                        
                        <button type="submit" :disabled="variationForm.processing" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 text-sm font-medium rounded-lg hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                            <i class="fas fa-plus" :class="{ 'fa-spinner fa-spin': variationForm.processing, 'fa-plus': !variationForm.processing }"></i> Adicionar Variação
                        </button>
                    </form>
                </div>
            </div>
