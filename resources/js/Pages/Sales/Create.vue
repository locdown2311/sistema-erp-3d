            ...
            <div class="overflow-hidden bg-white border shadow-sm dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 rounded-xl mb-6">
                ...
                <form @submit.prevent="submitForm" class="p-6">
                    <div class="grid grid-cols-1 gap-5 mb-5 md:grid-cols-3">
                        <div>
                            <label class="block mb-1.5 text-sm font-medium text-zinc-700 dark:text-zinc-300">Cliente</label>
                            <input type="text" v-model="form.customer_name" placeholder="Nome do cliente (opcional)" 
                                class="w-full px-4 py-2 text-sm transition-shadow border rounded-lg outline-none bg-zinc-50 dark:bg-zinc-950 border-zinc-300 dark:border-zinc-800 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 placeholder-zinc-400 dark:placeholder-zinc-600">
                            <span v-if="form.errors.customer_name" class="mt-1 text-xs text-red-500">{{ form.errors.customer_name }}</span>
                        </div>
                        <div>
